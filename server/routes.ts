...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Progressive loading endpoint - returns tokens immediately without prices
  app.get("/api/wallet/:address/progressive", async (req, res) => {
    try {
      const { address } = req.params;
      ...
      console.log(`Progressive loading for ${address} - returning basic token data immediately`);
      
      // Import progressive balance service
      const { getProgressiveBalances } = await import('./services/progressive-balance-service.js');
      
      // Get basic token data FAST
      const tokens = await getProgressiveBalances(address);
      
      // Return immediately with basic data
      return res.json({
        address,
        tokens,
        totalValue: 0, // Will be calculated on frontend as prices load
        tokenCount: tokens.length,
        plsBalance: tokens.find(t => t.isNative)?.balanceFormatted || 0,
        plsPriceChange: null,
        networkCount: 1,
        progressive: true,
        message: 'Basic token data loaded - prices loading in background'
      });
    } catch (error) {
  ...
  // Endpoint to enrich a single token with price and logo
  app.post("/api/token/enrich", async (req, res) => {
    try {
      ...
      // Import progressive balance service
      const { enrichTokenData, fetchTokenLogoFromDexScreener } = await import('./services/progressive-balance-service.js');
      
      // Enrich the token with price and logo
      ...
      return res.status(500).json({ 
        message: "Failed to enrich token",
        error: error instanceof Error ? error.message : "Unknown error" 
      });
    }
  });
  
  // Batch endpoint to enrich multiple tokens
  app.post("/api/tokens/enrich-batch", async (req, res) => {
    try {
      ...
      // Import progressive balance service
      const { enrichTokenData } = await import('./services/progressive-balance-service.js');
      
      // Enrich tokens in parallel (limit batch size)
      const batchSize = 5;
      ...
      for (let i = 0; i < tokens.length; i += batchSize) {
        ...
        const enrichedBatch = await Promise.all(
          batch.map(token => enrichTokenData(token))
        );
        enrichedTokens.push(...enrichedBatch);
      }
      
      return res.json({ tokens: enrichedTokens });
    } catch (error) {
      console.error("Error enriching tokens batch:", error);
      return res.status(500).json({ 
        message: "Failed to enrich tokens",
        error: error instanceof Error ? error.message : "Unknown error" 
      });
    }
  });
