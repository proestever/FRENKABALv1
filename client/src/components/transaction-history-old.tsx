...
export function TransactionHistory({ walletAddress, onClose }: TransactionHistoryProps) {
  ...
  const getSwapDetails = (tx: Transaction) => {
    ...
    if (!isSwap && !isMulticall && !isDexRouter) {
      return null;
    }

    // Process ERC20 transfers to show actual tokens going in/out
    if (tx.erc20_transfers && tx.erc20_transfers.length > 0) {
                ...
                <td className="px-6 py-4">
                  <div className="text-sm font-semibold text-white">
                    ...
                    {(() => {
                      ...
                      <div key={`${tx.hash}-erc20-${i}`} className="flex items-start mt-3 p-2 rounded-lg bg-black/20 border border-white/5">
                        <TokenLogo 
                          ...
                          <div className="mt-1 space-y-1">
                            ...
                            <div className="flex items-center gap-2 text-sm">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  onClick={() => copyToClipboard(transfer.address || '', `token-${i}`)}
                                  className="text-gray-400 hover:text-white transition-colors"
                                  title="Copy contract address"
                                >
                                  {copiedAddresses[`token-${i}`] ? (
