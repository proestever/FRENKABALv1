...
export function WalletShareModal({
  ...
  // Function to download wallet snapshot
  const downloadSnapshot = () => {
    ...
    const snapshot = {
      ...
      tokens: visibleTokens.map((token: any) => ({
    ...
    const exportFileDefaultName = `wallet_${walletAddress.slice(0, 8)}_${new Date().toISOString().split('T')[0]}.json`;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl w-[90vw] max-h-[90vh] bg-gradient-to-b from-black via-gray-950 to-black border-gray-800 text-white p-0 overflow-hidden [&>button]:text-gray-400 [&>button]:hover:text-white [&>button]:absolute [&>button]:right-4 [&>button]:top-4 [&>button]:z-20">
