...
export function ShareWalletCard({ wallet, portfolioName, tokens, hexStakesSummary }: ShareWalletCardProps) {
  ...
  const loadImage = (src: string): Promise<HTMLImageElement> => {
    return new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      img.onerror = () => {
        // If loading through proxy failed and it was an external URL, try direct loading
        if (imageSrc !== src && src.startsWith('http')) {
          ...
          directImg.crossOrigin = "anonymous"; // Try with CORS headers
          directImg.onload = () => resolve(directImg);
          directImg.onerror = () => {
            // Last resort - try without crossOrigin
            console.warn(`Direct image loading with CORS failed for ${src}, trying without CORS`);
            const lastAttemptImg = document.createElement('img');
            lastAttemptImg.onload = () => resolve(lastAttemptImg);
            lastAttemptImg.onerror = () => reject(new Error(`All attempts to load image failed: ${src}`));
            lastAttemptImg.src = src;
          };
          directImg.src = src;
        } else {
