...
export function PortfolioShareModal({ 
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl w-[95vw] sm:w-[90vw] max-h-[95vh] bg-gradient-to-b from-black via-gray-950 to-black border-gray-800 text-white p-0 overflow-hidden [&>button]:text-gray-400 [&>button]:hover:text-white [&>button]:absolute [&>button]:right-4 [&>button]:top-4 [&>button]:z-20">
        ...
        <div className="relative z-10 px-3 py-3 sm:px-4 sm:py-4 md:p-6 lg:p-8">
          ...
          <div className="flex items-center justify-center mb-3 sm:mb-4 md:mb-6">
            <img 
              ...
              alt="Frenkabal" 
              className="h-10 w-10 sm:h-12 sm:w-12 md:h-14 md:w-14 lg:h-16 lg:w-16 mr-2 sm:mr-3 md:mr-4 animate-pulse"
            />
            <div>
              <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
                FRENKABAL
              </h1>
          ...
          <div className="text-center mb-3 sm:mb-4 md:mb-6">
            ...
            <p className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-purple-400 bg-clip-text text-transparent">
          ...
          <div className="space-y-1.5 sm:space-y-2 md:space-y-3 mb-3 sm:mb-4 md:mb-6">
            {topTokens.map((token, index) => (
              <div key={token.address} className="flex items-center gap-1.5 sm:gap-2 md:gap-3 p-1.5 sm:p-2 md:p-3 rounded-lg bg-gray-900/50 backdrop-blur-sm border border-gray-800">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs sm:text-sm md:text-base font-semibold text-white uppercase tracking-wide">
                      {token.isLp && token.lpToken0Symbol && token.lpToken1Symbol 
                        ? `${token.lpToken0Symbol}/${token.lpToken1Symbol}`
                        : token.symbol || 'Unknown'}
                    </span>
                    <span className="text-xs sm:text-sm md:text-base font-bold text-green-400">
          ...
          <div className="text-center">
            <p className="text-gray-500 text-[0.65rem] sm:text-xs md:text-sm mb-1">
              Portfolio: {portfolioName}
            </p>
            <p className="text-gray-600 text-[0.65rem] sm:text-xs">
